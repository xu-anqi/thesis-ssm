...
#save dataset for sample_did
file_path <- "sample_did.RData"

save(cohabit_data, file = file_path)
...
rm(list=ls())
#load("C:/Users/AnqiXu/OneDrive - KU Leuven/Thesis/data analysis/sample_did.RData")
load("/Users/angelxu/Library/CloudStorage/OneDrive-KULeuven/Thesis/data analysis/sample_did.RData")
...
#!!!temporarily exclude non-working sample
summary(cohabit_data$INCWAGE)
cohabit_data <- cohabit_data %>%
  filter(INCWAGE>0)
summary(cohabit_data$INCWAGE)
cohabit_data$logINC <- log(cohabit_data$INCWAGE)

#hourly wage
cohabit_data$hourly_wage <- cohabit_data$INCWAGE/cohabit_data$annualHRSWORK
...
did_male <- lm(logINC ~ same_sex+legalized+same_sex*legalized
...
did_female <- lm(logINC ~ same_sex+legalized+same_sex*legalized
               +AGE+AGE_square+NCHILD+has_children_under5+factor(RACE_RECODED)
               +factor(HISPAN_RECODED)+factor(METRO_RECODED)+factor(EDUC_RECODED)
               +INCWAGE_partner+factor(YEAR)+factor(STATEICP), 
...
did_female <- lm(hourly_wage ~ same_sex+legalized+same_sex*legalized
                 +AGE+AGE_square+NCHILD+has_children_under5+factor(RACE_RECODED)
                 +factor(HISPAN_RECODED)+factor(METRO_RECODED)+factor(EDUC_RECODED)
                 +INCWAGE_partner+factor(YEAR)+factor(STATEICP), 
                 data = cohabit_data_female)
