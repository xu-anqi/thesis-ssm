...
global OUTPUT   = "$DIR/output/2_inten_inc"			// path for output
/*-----------------------------------------------------------------------------------
			/* Load Data */
-----------------------------------------------------------------------------------*/
...
foreach s in 1 2 {
	...
	drop if (incwage_99 == 0)
	gen ln_income = ln(incwage_99)
    ...
    * (1) baseline
     reghdfe ln_income i.same_sex##i.legal ///
        [pweight = perwt], vce(cluster statefip)
    eststo lpm1


    * (2) + demo
     reghdfe ln_income i.same_sex##i.legal age age_sq nchild ///
    ...
    * (3) + edu
	...
	(4) + home ownership, , non-labor income
     probit hours_year i.same_sex##i.legal age age_sq nchild ///
        i.nchlt5 i.race i.hispan i.edu ///
    ...
    * Export one table per sex
    esttab lpm1 lpm2 lpm3 /*probit4*/ lpm5 using "${OUTPUT}/did_`slbl'.txt", ///
        ...
        title("DID: labour income `=strproper("`slbl'")'") ///
