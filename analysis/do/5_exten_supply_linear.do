...
foreach s in 1 2 {
    ...
    * (1) baseline
    reghdfe has_supply i.same_sex##i.legal ///
        , vce(cluster statefip)
    eststo probit1


    * (2) + demo
     reghdfe has_supply i.same_sex##i.legal age age_sq nchild ///
    ...
    * Export one table per sex
    esttab probit1 probit2 probit3 probit5 /*probit4 probit6*/ using "${OUTPUT}/linear_did_`slbl'.txt", ///
        replace se b(3) star(* 0.10 ** 0.05 *** 0.01) ///
        title("DID: Labor Supply, Extensive Margin `=strproper("`slbl'")'") ///
