...
global OUTPUT   = "$DIR/output/mech-supply-primary/all"			// path for output
/*-----------------------------------------------------------------------------------
			/* Load Data */
-----------------------------------------------------------------------------------*/
...
foreach p in 0 1  {
	local plbl = cond(`p'==1, "primary earner", "secondary earner" )
...
foreach s in 1 2 {
	...
	drop if missing(incwage, incwage_partner)
	drop if incwage == incwage_partner
	gen primary_earner = (incwage > incwage_partner)
	keep if primary_earner == `p'
    ...
    * Export one table per sex per primary/secondary
    esttab lpm1 lpm2 lpm3 /*probit4*/ lpm5 using "${OUTPUT}/did_`plbl'_`slbl'.txt", ///
        ...
        title("DID: Labor Supply Intensive Margin, `=strproper("`plbl'")' , `=strproper("`slbl'")'") ///
